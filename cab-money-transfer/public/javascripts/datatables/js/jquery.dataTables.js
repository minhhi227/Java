...
(/** @lends <global> */function($, window, document, undefined) {
	...
	DataTable.defaults = {
			...
			oSettings.jqXHR = $.ajax( {
				...
				"success": function (json, textStatus, request) {
					if(request.getResponseHeader('Content-Length') == '0'){
						alert("L'exécution de votre requête a expirée. Veuillez réessayer");
						$.unblockUI();
					}
					else{
				...
				"error": function (xhr, error, thrown) {
					if ( error == "parsererror" ) {
						...
						oSettings.oApi._fnLog( oSettings, 0, "L'exécution de votre requête a expirée. Veuillez réessayer.");
						$.unblockUI();
					}
				}
			} );
		...
		"oLanguage": {
			 ...
			 *    } );
			 */
			"sInfoFiltered": "(filtré _MAX_ total des entrées)",
